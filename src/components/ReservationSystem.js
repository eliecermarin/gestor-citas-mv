...
export default function ReservationSystem({ businessId }) {
  ...
  useEffect(() => {
    const loadBusinessData = async () => {
      ...
      try {
        ...
        console.log('🔄 Cargando datos para negocio:', currentBusinessId);
  ...
  const sendReservationEmails = async (reservaCreada, businessId) => {
    try {
      ...
      const trabajadorSeleccionado = workers.find(w => w.id === reservaCreada.trabajador);
      ...
      console.log('📧 Config del negocio:', businessConfig);
      ...
      const emailData = {
        ...
        isPublicReservation: true
      };

      console.log('📧 Datos para email:', emailData);

      // 📧 ENVIAR EMAILS
      console.log('📧 LLAMANDO A /api/send-reservation-emails...');
      ...
      console.log('📧 Resultado de emails:', result);

      if (result.success) {
        console.log('✅ Emails enviados exitosamente');
        return { success: true, emailsSent: result.emailsSent, details: result.details };
      } else {
  ...
  const submitReservation = async () => {
    ...
    console.log('💾 CREANDO RESERVA EN SUPABASE...');
    ...
    return { 
      success: true, 
      data: data  // ✅ IMPORTANTE: Retornar los datos de la reserva
    };
  };
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const emailResult = await sendReservationEmails(result.data, currentBusinessId);
